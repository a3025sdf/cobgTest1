...
#         # k, v: [batch_size, seq_len_k, embed_dim]
#         batch_size = q.size(0)
#         seq_len = q.size(1)
#         # 拆分多头
#         q = self.to_q(q).view(-1, q.size(1), self.num_heads, self.head_dim).transpose(1, 2)
#         k = self.to_k(k).view(-1, k.size(1), self.num_heads, self.head_dim).transpose(1, 2)
#         v = self.to_v(v).view(-1, v.size(1), self.num_heads, self.head_dim).transpose(1, 2)
#
#         # 计算注意力分数
...
class TPLinkerBert(nn.Module):
    def __init__(self, encoder, 
        ...
        # 外侧似乎起到的是一个分类器的作用，kernel是起到输出标记的作用?
        ...
        self.to_ent = nn.Linear(hidden_size,hidden_size)
        self.lstm_2ent = nn.Linear(2*LSTM_hiddensize,hidden_size)
        # handshaking kernel
        ...
        if if_check==True:
            self.check_head_rel_fc = nn.Linear(hidden_size + 3, 2)
            self.final_head_rel_fc = nn.Linear(hidden_size + 3 + 2, 3)
        self.LSTM = nn.LSTM(
            hidden_size,LSTM_hiddensize,2,batch_first=True,dropout=0.2,bidirectional=True
        )
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        last_hidden_state_2r = context_outputs[0]
        last_hidden_state_2e = context_outputs[0]
        last_hidden_state_2e = self.to_ent(last_hidden_state_2e)
        last_hidden_state_2e,_ = self.LSTM(last_hidden_state_2e)
        last_hidden_state_2e = self.lstm_2ent(last_hidden_state_2e)
        #todo 如果把关系作为因素的话，尝试在上三角那块作为

        batch_size = last_hidden_state_2r.size(0)
        seq_len = last_hidden_state_2r.size(1)
        q = self.to_q(last_hidden_state_2r).view(-1, last_hidden_state_2r.size(1), self.num_heads,
                                                    self.head_dim).transpose(1, 2)
        k = self.to_k(last_hidden_state_2r).view(-1, last_hidden_state_2r.size(1), self.num_heads,
                                                    self.head_dim).transpose(1, 2)
        v = self.to_v(last_hidden_state_2r).view(-1, last_hidden_state_2r.size(1), self.num_heads,
                                                    self.head_dim).transpose(1, 2)
        ...
        last_hidden_state_2r =last_hidden_state_2r + self.out_proj(attn_output)


        # shaking_hiddens: (batch_size, 1 + ... + seq_len, hidden_size)
        # shaking_hiddens = self.handshaking_kernel(last_hidden_state)
        shaking_hiddens4ent = self.handshaking_kernel(last_hidden_state_2e)
        shaking_hiddens4rel = self.handshaking_kernel(last_hidden_state_2r)
        #
        # batch_size = shaking_hiddens.size(0)
        # seq_len = shaking_hiddens4ent.size(1)
        # # 拆分多头
        # q = self.to_q_e2r(shaking_hiddens4ent).view(-1, shaking_hiddens4ent.size(1), self.num_heads,
        #                                             self.head_dim).transpose(1, 2)
        # k = self.to_k_e2r(shaking_hiddens4rel).view(-1, shaking_hiddens4rel.size(1), self.num_heads,
        #                                             self.head_dim).transpose(1, 2)
        # v = self.to_v_e2r(shaking_hiddens4rel).view(-1, shaking_hiddens4rel.size(1), self.num_heads,
        #                                             self.head_dim).transpose(1, 2)
        #
        # # 计算注意力分数
        # attn_scores = (q @ k.transpose(-2, -1)) / (self.head_dim ** 0.5)
        ...
        # # 输出投影
        #
        # shaking_hiddens4ent2 = self.out_proj_e2r(attn_output)
        #
        # q = self.to_q_r2e(shaking_hiddens4rel).view(-1, shaking_hiddens4rel.size(1), self.num_heads,
        #                                             self.head_dim).transpose(1, 2)
        # k = self.to_k_r2e(shaking_hiddens4ent).view(-1, shaking_hiddens4ent.size(1), self.num_heads,
        #                                             self.head_dim).transpose(1, 2)
        # v = self.to_v_r2e(shaking_hiddens4ent).view(-1, shaking_hiddens4ent.size(1), self.num_heads,
        #                                             self.head_dim).transpose(1, 2)
        #
        # # 计算注意力分数
        # attn_scores = (q @ k.transpose(-2, -1)) / (self.head_dim ** 0.5)
        ...
        # # 输出投影
        #
        # shaking_hiddens4rel2 = self.out_proj_r2e(attn_output)
        #
        # shaking_hiddens4ent = shaking_hiddens4ent2
        # shaking_hiddens4rel = shaking_hiddens4rel2
        # shaking_hiddens4ent2 = None
        # shaking_hiddens4rel2 = None
...
#             shaking_hiddens4rel = shaking_hiddens
        ...
        # if if_check==True:
        #     sent_ref = shaking_hiddens.unsqueeze(1).repeat(1, head_rel_shaking_outputs.size(1), 1, 1)
        #     check_head_rel = self.check_head_rel_fc(torch.cat(
        #         (sent_ref,
        #          head_rel_shaking_outputs)
        #         , dim=-1))
        #     head_rel_shaking_outputs = self.final_head_rel_fc(
        #         torch.cat((sent_ref, head_rel_shaking_outputs, check_head_rel), dim=-1))
